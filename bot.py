...
def callback_handler(call):
   try:
       ...
       if data == "cat_tech":
           bot.edit_message_text("🛒 Catégorie Tech", chat_id, call.message.message_id, reply_markup=tech_menu())
       elif data == "cat_fournisseur":
           bot.edit_message_text("📦 Catégorie Fournisseur", chat_id, call.message.message_id, reply_markup=fournisseur_menu())
       elif data == "back_main":
           bot.edit_message_text("🔥 Menu Principal", chat_id, call.message.message_id, reply_markup=main_menu())
       elif data.startswith("tech_") or data.startswith("four_"):
           bot.answer_callback_query(call.id, "✅ Sélectionné")
           bot.send_message(chat_id, f"Tu as sélectionné : {data}\n\nContacte le support pour le prix et paiement (@UHQ_7500).")
       elif data == "support":
